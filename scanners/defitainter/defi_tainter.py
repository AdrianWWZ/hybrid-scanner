...
class Contract:
    ...
    def set_url(self):
        ...
        url_map = {
            ...
            "Avalanche": os.getenv("AVAX_RPC_URL"),
            "Polygon":   os.getenv("POLYGON_RPC_URL"),
            "Solana":    os.getenv("SOLANA_RPC_URL"),
            "Fantom":    os.getenv("FANTOM_RPC_URL"),
            "Gnosis":    os.getenv("GNOSIS_RPC_URL"),
        }
        self.url = url_map.get(self.platform)
        if not self.url:
            raise ValueError(
                f"No RPC URL configured for platform '{self.platform}'. "
                f"Set the corresponding *_RPC_URL in your .env file."
            )
